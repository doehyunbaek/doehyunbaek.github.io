...
- Keyboard shortcuts: `/` focus event search, `j` future, `k` past, `t` current time, `p` open Add paper modal, `a` all calendars, `q`/`w`/`e`/`r` solo calendars 1–4, `Ctrl+1`–`Ctrl+3` sidebar panels, `1` deadlines, `2` week, `3` month, `4` four weeks, `5` heatmap, `Backspace` delete focused edit event
