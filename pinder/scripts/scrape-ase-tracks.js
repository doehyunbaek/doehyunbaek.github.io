...
const ASE_DATA_PATH = path.join(ROOT_DIR, 'data', 'ase.json');
...
const ENABLE_DOI_FALLBACK = process.env.PINDER_ENABLE_DOI_FALLBACK === '1';
...
async function loadAseCollection() {
  return JSON.parse(await fs.readFile(ASE_DATA_PATH, 'utf8'));
}

async function saveAseCollection(collection) {
  const nextCollection = {
    ...collection,
    updatedAt: new Date().toISOString(),
  };
  await fs.writeFile(ASE_DATA_PATH, JSON.stringify(nextCollection, null, 2));
}
...
function mergeTrackPapers(existingPapers = [], nextPapers = []) {
  ...
  const nextPaperIds = new Set(nextPapers.map((paper) => String(paper?.id || '')));

  const mergedPapers = nextPapers.map((nextPaper) => {
  ...
  existingPapers.forEach((existingPaper) => {
    const existingPaperId = String(existingPaper?.id || '');
    if (existingPaper?.preserveOnRescrape && existingPaperId && !nextPaperIds.has(existingPaperId)) {
      mergedPapers.push(existingPaper);
    }
  });
...
async function main() {
  ...
  const collection = await loadAseCollection();
  const tracks = selectTracks(collection, selectedTrackKeys);

  if (!tracks.length) {
    throw new Error(`No ASE tracks matched: ${Array.from(selectedTrackKeys).join(', ') || '(none)'}`);
  }
      ...
      try {
        ...
        let payload = await page.evaluate(async (trackEntry) => {
          ...
          return window.PinderScraper[scraperName]({
            ...
            includeTitlePatterns: Array.isArray(trackEntry.includeTitlePatterns)
              ? trackEntry.includeTitlePatterns
              : [],
            excludeTitlePatterns: Array.isArray(trackEntry.excludeTitlePatterns)
              ? trackEntry.excludeTitlePatterns
              : [],
            publicationYear: Number(trackEntry.year) || 0,
            concurrency: trackEntry.scraper === 'dblp' ? 4 : 8,
            excludeBadgeLabels: Array.isArray(trackEntry.excludeBadgeLabels)
              ? trackEntry.excludeBadgeLabels
              : [],
        ...
        const currentCollection = await loadAseCollection();
        currentCollection.tracks = (currentCollection.tracks || []).map((entry) => {
          ...
          return {
            ...
            includeTitlePatterns: Array.isArray(track.includeTitlePatterns)
              ? track.includeTitlePatterns
              : (Array.isArray(entry.includeTitlePatterns) ? entry.includeTitlePatterns : []),
            excludeTitlePatterns: Array.isArray(track.excludeTitlePatterns)
              ? track.excludeTitlePatterns
              : (Array.isArray(entry.excludeTitlePatterns) ? entry.excludeTitlePatterns : []),
            excludeBadgeLabels: Array.isArray(track.excludeBadgeLabels)
              ? track.excludeBadgeLabels
              : (Array.isArray(entry.excludeBadgeLabels) ? entry.excludeBadgeLabels : []),
        ...
        await saveAseCollection(currentCollection);
        console.log(`Updated data/ase.json → ${track.slug} (${payload.paperCount} papers)`);
      } finally {
        await page.close();
      }
    }
